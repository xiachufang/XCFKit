...
static inline NSTimeInterval XCFVideoRangeGetEnd(XCFVideoRange range) {
  return range.location + range.length;
}

static inline BOOL XCFVideoRangeEqualToRange(XCFVideoRange range1,
                                             XCFVideoRange range2) {
  return range1.location == range2.location && range1.length == range2.length;
}

static inline XCFVideoRange XCFVideoRangeMake(NSTimeInterval start,
                                              NSTimeInterval end) {
