...
@interface XCFVideoRangeSlider () <UICollectionViewDataSource, UICollectionViewDelegate, UIGestureRecognizerDelegate>
...
- (void)loadVideoFramesWithVideoAsset:(AVAsset *)asset {
    ...
    [self.asset loadValuesAsynchronouslyForKeys:@[loadKey]
                              completionHandler:^{
...
- (void)asycGenerateThumbnailImageAtIndex:(NSInteger)requestIndex
    ...
    if (resultImage) {
        if (result)
            result(requestIndex, resultImage);
        return;
    }
    ...
    for (NSInteger idx = requestIndex; idx < MIN(totalFrames, requestIndex + 20); idx += 1) {
        NSNumber *key = @(idx);
        if ([self.cachedFrames objectForKey:key] || [self.runningTasks objectForKey:key])
            continue;
    ...
    __weak typeof(self) weak_self = self;
    [self.imageGenerator generateCGImagesAsynchronouslyForTimes:requestTimeValues
                                              completionHandler:^(CMTime requestedTime, CGImageRef _Nullable image, CMTime actualTime, AVAssetImageGeneratorResult r, NSError *_Nullable error) {
                                                  __strong typeof(weak_self) strong_self = weak_self;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [self asycGenerateThumbnailImageAtIndex:item
                                     result:^(NSInteger idx, UIImage *image) {
                                         if (cell.tag == idx) {
...
- (BOOL)isTracking {
    return ([super isTracking] && CGRectContainsPoint(self.bounds, _trackInitPoint)) ||
...
- (BOOL)beginTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
    if (self.videoLength < self.minimumTrimLength)
        return NO;
