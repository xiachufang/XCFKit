...
- (void)loadView {
    ...
    [self.videoAsset loadValuesAsynchronouslyForKeys:@[loadkey]
                                   completionHandler:^{
...
- (void)_pauseVideo:(id)sender {
    if (self.videoRangeSlider.isTracking)
        return;
...
#endif
        [self.playerView asyncSeekToSecond:targetSecond
...
- (void)expertVideo:(id)sender {
    ...
    CGPoint cropVideoOrigin;
    CGSize continerSize = self.playerScrollView.contentSize;
    CGSize playerContainerSize = [self _playerContainerFrame].size;
...
- (void)avPlayerViewDidPlayToEnd:(XCFAVPlayerView *)playerView {
    [playerView pause];
    [playerView asyncSeekToSecond:self.currentRange.location
                       completion:^(BOOL finish) {
                           [playerView play];
                       }];
}
